...
Once the population and reaction elements are specified, defining a stochastic trajectory
in ReMASTER's XML is very easy.

For example, the following represents a stochastic birth-death-sampling process
trajectory with constant rates:
...
The =maxTime= attribute specifies the maximum end time of the simulation.  For stochastic trajectories
this attribute is actually optional, since there are many birth-death processes (although not this one)
are guaranteed to terminate in finite amount of time.  (ReMASTER always ceases the simulation once
no reactions are able to fire.)
...
#+begin_src xml
  <beast version="2.0" namespace="beast.core.parameter:remaster">
    <run spec="Simulator" nSims="100">
      <simulate spec="StochasticTrajectory" maxTime="10">
        <population id="X" spec="RealParameter" value="1"/>
        <reaction spec="Reaction" rate="1.2"> X -> 2X </reaction>
        <reaction spec="Reaction" rate="1"> X -> 0 </reaction>
      </simulate>
...
In order to actually visualise the simulation output, we can make use of the script
...
#+end_example
Each row specifies the size of a particular population (and

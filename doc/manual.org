...
#+begin_src xml
<beast version="2.8" namespace="beast.base.spec.inference.parameter:remaster">
    <run spec="Simulator" nSims="100">
        <simulate spec="SimulatedTree" id="tree">
            <trajectory spec="StochasticTrajectory" id="SIRTrajectory" endsWhen="S>=10">
                <population spec="RealVectorParam" id="X" value="1"/>
                <samplePopulation spec="RealVectorParam" id="S" value="0"/>
...
** Citing ReMASTER
:PROPERTIES:
:CUSTOM_ID: id-2-Overview-Citing-ReMASTER
:END:
...
recent version of BEAST 2 (version 2.8 or later) is available on your system.
...
Note that the birth-death models ReMASTER is capable of simulating are
restricted to those whose population dynamics obey a time-inhomogeneous
Chemical Master Equation.  More details on this are given below when
discussing the connection between reaction rates and propensities.
...
A population in ReMASTER is simply a ~RealVectorParam~ (or other
~RealVector~) with a specific ID.  The value of the ~RealVectorParam~ is
...
of 10:
#+begin_src xml
<population id="X" spec="RealVectorParam" value="10"/>
#+end_src

If the ~RealVectorParam~ has more than one element, these represent subpopulations.
For example, the following population "Y" has two subpopulations initially
occupied by 1 and 10 individuals, respectively:
#+begin_src xml
<population id="Y" spec="RealVectorParam" value="1 10"/>
#+end_src

Populations may also be marked as "sample" populations by using the appropriate
tag:
#+begin_src xml
<samplePopulation id="sample" spec="RealVectorParam" value="0"/>
#+end_src
...
of the system, determine the reaction probability per
...
The relationship between reaction rate $\lambda$ and the overall
propensity takes the form used by so-called Chemical Master Equations,
where the dependence of the propensity on the current population sizes
is determined entirely by mass action kinetics.  Just as in MASTER before
it, ReMASTER cannot simulate birth-death processes where propensities have
some other dependence on the population sizes.
...
*** Complex model configurations using plates
:PROPERTIES:
:CUSTOM_ID: id-3-Model-specification-Birth-death-simulations-Complex-model-configurations-using-plates
:END:
...
#+begin_src xml
  <population id="X" spec="RealVectorParam" value="1"/>
  <population id="Y" spec="RealVectorParam" value="1"/>
  <population id="Z" spec="RealVectorParam" value="1"/>
  <population id="W" spec="RealVectorParam" value="1"/>
  <population id="V" spec="RealVectorParam" value="1"/>

  <reaction spec="Reaction" rate="1.0"> X -> 2X </reaction>
  <reaction spec="Reaction" rate="1.0"> Y -> 2Y </reaction>
  <reaction spec="Reaction" rate="1.0"> Z -> 2Z </reaction>
  <reaction spec="Reaction" rate="1.0"> W -> 2W </reaction>
  <reaction spec="Reaction" rate="1.0"> V -> 2V </reaction>
#+end_src
...
#+begin_src xml
  <plate var="pop" range="X,Y,Z,W,V">
    <population id="$(pop)" spec="RealVectorParam" value="1"/>
    <reaction spec="Reaction" rate="1.0"> $(pop) -> 2$(pop) </reaction>
  </plate>
#+end_src
...
#+begin_src xml
  <plate var="source" range="X,Y,Z,W,V">
    <plate var="dest" range="X,Y,Z,W,V">
      <reaction spec="Reaction" rate="0.5"> $(source) -> $(dest) </reaction>
    </plate>
  </plate>
#+end_src
...
#+begin_src xml
  <population id="X" spec="RealVectorParam" dimension="100" value="1"/>
  <plate var="i" range="0:99">
    <reaction spec="Reaction" rate="1.0"> X[$(i)] -> 2X[$(i)] </reaction>
  </plate>
#+end_src
...
attributes define the maximum size of the steps taken by the
forward-time and backward-time numerical integrators, respectively,
relative to the total simulation time. Reducing the size of these
values increases the accuracy of the deterministic trajectory
calculation and the associated tree simulation, but makes the
calculation more computationally demanding.  The forward-time step
size affects the population size trajectory calculation accuracy,
while the backward-time step size affects only the subsequent tree
simulation accuracy.  Leaving these values to their default settings
is sometimes okay, but smaller step sizes are often necessary to avoid
numerical errors. These sometimes manifest in the form of "too many
lineages remaining" or "step reaction probability exceeds 1" errors,
but may also affect the output in more subtle ways.  Models where
populations are changing very quickly are particularly susceptible to
such problems.  It is thus important to try various values of these
tuning parameters to ensure numerical convergence.
...
- nSims (required) :: an integer specifying the number of independent simulations to perform,
- simulate (required) :: an object representing the simulation, in this case a =StochasticTrajectory=, and
...
#+begin_src xml
  <tree spec="SimulatedTree">
    <trajectory spec="StochasticTrajectory"
                maxTime="10">
      <population id="X" spec="RealVectorParam" value="1"/>
      <samplePopulation id="samp" spec="RealVectorParam" value="0"/>
...
- trajectory (required) :: a ReMASTER trajectory object.
...
- maxRetries (default 10) :: This is the maximum number of times (default 10) that
  ...
  individuals in the sampled population is greater than 1.)

- removeSingletonNodes (default false) ::
  This can be used to specify that single child nodes representing type changes
...
data set, as demonstrated in the [[MCMCInitExample][YuleInferenceInitialisation.xml]] example.
  ...
  <beast version="2.8" namespace="beast.base.spec.inference.parameter:remaster">
    <run spec="Simulator" nSims="1">
      <simulate id="tree" spec="SimulatedTree">
        <trajectory id="trajectory" spec="StochasticTrajectory" maxTime="10">
          <population id="X" spec="RealVectorParam" value="1"/>
          <population id="Y" spec="RealVectorParam" value="0"/>
          <samplePopulation id="samp" spec="RealVectorParam" value="0"/>
...
*** Logging trees with variable numbers of leaves
:PROPERTIES:
:CUSTOM_ID: id-3-Tree-simulation-Birth-death-simulations-Logging-trees-with-variable-numbers-of-leaves
:END:
...
*** Logging both full and sampled trees
:PROPERTIES:
:CUSTOM_ID: id-3-Tree-simulation-Birth-death-simulations-Logging-both-full-and-sampled-trees
:END:

Occasionally it is useful to record the complete tree together with a
sampled tree in which lineages not ancestral to samples have been
pruned.

This can be accomplished through the use of the =PrunedTree= class.
This takes a =SimulatedTree= together with one or more sample
populations as input, and produces a representation of the input tree
on which the lineages not ancestral to the specified sample
populations have been removed (pruned).

For example, the following produces two output files, ~output.full.trees~
and ~output.sampled.trees~, which contain the full and sampled trees
respectively.  (Note the use of an additional "removed" sample population.)
#+begin_src xml
  <beast version="2.8" namespace="beast.spec.base.inference.parameter:remaster">
    <run spec="Simulator" nSims="1">
      <simulate id="tree" spec="SimulatedTree">
        <trajectory id="trajectory" spec="StochasticTrajectory" maxTime="10">
          <population id="X" spec="RealVectorParam" value="1"/>
          <population id="Y" spec="RealVectorParam" value="0"/>
          <samplePopulation id="removed" spec="RealVectorParam" value="0"/>
          <samplePopulation id="sampled" spec="RealVectorParam" value="0"/>

          <reaction spec="Reaction" rate="1.4"> X -> 2X </reaction>
          <reaction spec="Reaction" rate="0.1"> X -> Y </reaction>
          <reaction spec="Reaction" rate="0.5"> X -> removed </reaction>
          <reaction spec="Reaction" rate="0.5"> Y -> removed + sampled </reaction>
        </trajectory>
      </simulate>

      <logger fileName="output.full.trees" mode="tree">
        <log spec="TypedTreeLogger">
          <tree spec="PrunedTree" samplePops="removed" simulatedTree="@tree"/>
        </log>
      </logger>
      <logger fileName="output.sampled.trees" mode="tree">
        <log spec="TypedTreeLogger">
          <tree spec="PrunedTree" samplePops="sampled" simulatedTree="@tree"/>
        </log>
      </logger>
    </run>
  </beast>
...
rather than regular ~RealVector~ objects. For instance, the following element
...
*** Vector-valued coalescent Reaction inputs
:PROPERTIES:
:CUSTOM_ID: id-3-Model-specification-Coalescent-simulations-Vector-valued-coalescent-Reaction-inputs
:END:
...
#+begin_src xml
<beast version="2.8"
       namespace="remaster:beast.base.evolution.tree.coalescent">
  <run spec="Simulator" nSims="1">
...
#+begin_src xml
<beast version="2.8"
       namespace="remaster:beast.base.evolution.tree.coalescent:feast.popmodels">
  <run spec="Simulator" nSims="1">
    <simulate id="tree" spec="SimulatedTree">
...
- it uses ~RealVectorParam~ or ~RealVector~ objects to define birth-death populations,
...
- the inputs defining reaction rates, times, probabilities or numbers
  expect generic ~RealVector~ inputs with appropriate domains.
...
Furthermore, the copious use of =RealVector= input types means that one can use
the =RealVector=-specific objects from the [[https://tgvaughan.github.io/feast][feast]] package to create simulation XMLs
...
follows:
#+begin_src xml
  <beast version="2.8" namespace="beast.base.spec.inference:remaster">
    <param id="R" spec="RealVectorParam" value="1.5"/>
    <param id="mu" spec="RealVectorParam" value="1.0"/>
...
done by supplying a =feast.realvector.Sequence= object (feast
documentation [[https://tgvaughan.github.io/feast/#id-3-Numeric-sequences-as-RealVectors][here]]), which generates a =RealVector= representing a
sequence of evenly spaced numbers, to the ~times~ input of the
corresponding =PunctualReaction=:

#+begin_src xml
 <beast version="2.8"
       namespace="beast.base.spec.inference.parameter:remaster
    ...
    <simulate id="tree" spec="SimulatedTree">
      <trajectory id="traj" spec="CoalescentTrajectory">
        ...
        <reaction spec="PunctualReaction" n="1">
          <times spec="feast.realvector.Sequence" start="0" stop="0.5" length="200"/>
          0 -> pop
        </reaction>
      </trajectory>
    </simulate>
...
**Note**: Some of these examples also require the BEAST 2 package [[https://tgvaughan.github.io/feast][=feast=]] to be installed.
...
- [[https://github.com/tgvaughan/remaster/blob/master/examples/BDserialAlignment.xml][=BDserialAlignment.xml=]] ::
  Simulates a birth-death-sampling trajectory and an alignment.
...
- [[https://github.com/tgvaughan/remaster/blob/master/examples/CoalescentInferenceAlignment.xml][=CoalescentInferenceAlignment.xml=]] (requires feast) ::
  ...
  simulated down the tree using the sequence simulator from the [[https://tgvaughan.github.io/feast][feast]]
...
- [[https://github.com/tgvaughan/remaster/blob/master/examples/SEIR_tree_pruning.xml][=SEIR_tree_pruning.xml=]] ::
  Simulates a tree under the stochastic
  Susceptible-Exposed-Infectious-Removed (SEIR) model, demonstrating
  the use of =PrunedTree= to record both full and sampled trees.
...
- <<MCMCInitExample>>[[https://github.com/tgvaughan/remaster/blob/master/examples/YuleInferenceInitialisation.xml][=YuleInferenceInitialisation.xml=]] :: 
